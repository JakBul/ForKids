...
ForKids target users are kids, adults, parents or collectors. Anyone who loves toys in general and would like to buy them. They would definitely look for:
...
This project is about the Full Stack development - so I decided to focus not only on the Back End and logic part of the website, but Front End as well.
...
I have added an option to either 'Update' the quantity or 'Remove' the product from the shopping bag if the user wants to do so. It is clearly visible how much is the purchase, delivery costs(if applied) and the grand total. Again, an extra button 'Keep Shopping' is provided to send the user back to products page.
...
This is page where users can register in order to create the profile on our webpage. All the program needs is e-mail address, chosen username and password. The user should get notification e-mail to verify their e-mail address.
